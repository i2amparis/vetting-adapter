...
class TestMultiHistoricalComparisonOutput(unittest.TestCase):
    ...
    def setUp(self):
        ...
        exact = HistoricalComparisonOutput(
            reference=reference, variable='Production|Chemicals|Plastics',
            region='EU27+UK+NO+CH',
        )
        ...
        no_fallback_reference = _idf([
            ['Historical', 'MS16', 'European Union (R9)', 'Population', 'million',
             2019, 446.06],
        ])
        no_fallback = HistoricalComparisonOutput(
            reference=no_fallback_reference, variable='Population',
            region='European Union (R9)',
        )
        self.multi = MultiHistoricalComparisonOutput({
            'plastics': (exact, fallback),
            'population': (no_fallback, None),
        })
    ###END def setUp
    ...
    def test_returns_exact_when_available(self):
        ...
        result = self.multi.get_applicable_output('plastics', checked)
        self.assertIsNotNone(result)
        _output, is_fallback = result
    ...
    def test_entry_without_fallback_returns_none_when_not_applicable(self):
        checked = _idf([
            ['modelA', 'scenA', 'World', 'GDP|MER', 'billion USD_2017/yr', 2020, 1.0],
        ])
        result = self.multi.get_applicable_output('population', checked)
        self.assertIsNone(result)
    ###END def test_entry_without_fallback_returns_none_when_not_applicable
